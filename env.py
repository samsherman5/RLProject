...
class PickPlaceEnv():
  ...
  def check_success(self):
    """ method to check if the final pose matches the ground truth"""
    ## "on(yellow1,blue1), on(yellow2, blue2), on(purple1, yellow1), on(purple2, yellow2), clear(purple1), clear(purple2), ontable(blue1), ontable(blue2)"    
    ...
    total_conditions_passed = 0

    if self.clear('purple1 block'):
      total_conditions_passed += 1
    if self.clear('purple2 block'):
      total_conditions_passed += 1      
    if self.on_table('blue1 block'):
      total_conditions_passed += 1      
    if self.on_table('blue2 block'):
      total_conditions_passed += 1          
    if self.on_top_of('yellow1 block', 'blue1 block'):
      total_conditions_passed += 1          
    if self.on_top_of('purple1 block', 'yellow1 block'):
      total_conditions_passed += 1          
    if self.on_top_of('yellow2 block', 'blue2 block'):
      total_conditions_passed += 1          
    if self.on_top_of('purple2 block', 'yellow2 block'):
      total_conditions_passed += 1            

    if total_conditions_passed == 8:
      return True
    return False